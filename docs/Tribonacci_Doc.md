...
The `ComputeTrib` method calculates the $n$-th Tribonacci number iteratively. The Tribonacci sequence is similar to the Fibonacci sequence, but each term is the sum of the _three_ previous terms rather than two. The sequence is defined recursively as follows:
...
| :------------------------- | :----- | :---------------------------------------------------------------------------------------------------------------------------- |
| `invariant a == Trib(i)`   | Failed | Misaligned indexing. After shifting, $a$ becomes `Trib(i)` only _after_ the increment. Dafny rejects it at the loop boundary. |
